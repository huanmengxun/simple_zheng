...
import com.zheng.swing.SwingTheme.EmeraldTheme;
import com.zheng.swing.SwingTheme.RubyTheme;
 ...
 * @author Jeff Dinkins
 */
public class MySwingEntry extends JPanel {
	/**
	 * 
	 */
	...
	// List of demos
	/** The demos list. */
	private ArrayList<MyDemoModule> demosList = new ArrayList<MyDemoModule>();
...
//	private Dimension VGAP = new Dimension(5, 1);
	...
	/** The current demo. */
	private MyDemoModule currentDemo = null;
	...
	/** The applet. */
	private SwingSetBaseApplet applet = null;
	...
	/** The swing sets. */
	private static Vector<MySwingEntry> swingSets = new Vector<MySwingEntry>();
	 ...
	 */
	public MySwingEntry(SwingSetBaseApplet applet) {
		this(applet, null);
	}

	/**
	 * MySwingSet Constructor.
	 *
	 * @param applet the applet
	 * @param gc     the gc
	 */
	public MySwingEntry(SwingSetBaseApplet applet, GraphicsConfiguration gc) {
...
//				getString("TabbedPane.src_label"),
				"Java源代码", null, scroller, getString("TabbedPane.src_tooltip"));
		
	}

	/** The current tab demo. */
	MyDemoModule currentTabDemo = null;

	/**
	 * The listener interface for receiving tab events. The class that is interested
	 * in processing a tab event implements this interface, and the object created
	 * with that class is registered with a component using the component's
	 * <code>addTabListener<code> method. When the tab event occurs, that object's
...
//				.__IconFactory__.getInstance().getRadioButtonMenuItemCheckIcon());
		...
		if (numSSs == 0) {
			...
			themesMenu.getAccessibleContext()
			...
			audioMenu.getAccessibleContext()
					.setAccessibleDescription(getString("AudioMenu.audio_accessible_description"));

			createAudioMenuItem(audioMenu, "AudioMenu.on_label", "AudioMenu.on_mnemonic",
					"AudioMenu.on_accessible_description", new OnAudioAction(this));

			mi = createAudioMenuItem(audioMenu, "AudioMenu.default_label", "AudioMenu.default_mnemonic",
					"AudioMenu.default_accessible_description", new DefaultAudioAction(this));
			mi.setSelected(true); // This is the default feedback setting

			createAudioMenuItem(audioMenu, "AudioMenu.off_label", "AudioMenu.off_mnemonic",
					"AudioMenu.off_accessible_description", new OffAudioAction(this));
			...
			fontMenu.getAccessibleContext()
					.setAccessibleDescription(getString("FontMenu.fonts_accessible_description"));
			ButtonGroup fontButtonGroup = new ButtonGroup();
			mi = createButtonGroupMenuItem(fontMenu, "FontMenu.plain_label", "FontMenu.plain_mnemonic",
					"FontMenu.plain_accessible_description", new ChangeFontAction(this, true), fontButtonGroup);
			mi.setSelected(true);
			mi = createButtonGroupMenuItem(fontMenu, "FontMenu.bold_label", "FontMenu.bold_mnemonic",
					"FontMenu.bold_accessible_description", new ChangeFontAction(this, false), fontButtonGroup);

			// *** now back to adding color/font themes to the theme menu
			mi = createThemesMenuItem(themesMenu, "ThemesMenu.ocean_label", "ThemesMenu.ocean_mnemonic",
					"ThemesMenu.ocean_accessible_description", new OceanTheme());
			mi.setSelected(true); // This is the default theme

			createThemesMenuItem(themesMenu, "ThemesMenu.steel_label", "ThemesMenu.steel_mnemonic",
					"ThemesMenu.steel_accessible_description", new DefaultMetalTheme());
			...
			mi = createCheckBoxMenuItem(optionsMenu, "OptionsMenu.tooltip_label", "OptionsMenu.tooltip_mnemonic",
					"OptionsMenu.tooltip_accessible_description", new ToolTipAction());
			mi.setSelected(true);

			// ***** create drag support submenu item.
			createCheckBoxMenuItem(optionsMenu, "OptionsMenu.dragEnabled_label", "OptionsMenu.dragEnabled_mnemonic",
					"OptionsMenu.dragEnabled_accessible_description", new DragSupportAction());
		...
		if (!isApplet()) {
			...
			if (screens.length > 1) {
				...
				multiScreenMenu.getAccessibleContext()
	 ...
	 */
	private JMenuItem createCheckBoxMenuItem(JMenu menu, String label, String mnemonic, String accessibleDescription,
			Action action) {
	 ...
	 * @return the j menu item
	 */
	private JMenuItem createButtonGroupMenuItem(JMenu menu, String label, String mnemonic, String accessibleDescription,
	 ...
	 * @return the j menu item
	 */
	public JMenuItem createAudioMenuItem(JMenu menu, String label, String mnemonic, String accessibleDescription,
			Action action) {
		JRadioButtonMenuItem mi = (JRadioButtonMenuItem) menu.add(new JRadioButtonMenuItem(getString(label)));
...
//		mi.setBorder(BorderFactory.createEmptyBorder());
	 ...
	 * @return the j menu item
	 */
	public JMenuItem createThemesMenuItem(JMenu menu, String label, String mnemonic, String accessibleDescription,
			MetalTheme theme) {
		JRadioButtonMenuItem mi = (JRadioButtonMenuItem) menu.add(new JRadioButtonMenuItem(getString(label)));
...
//		mi.setEnabled(isAvailableLookAndFeel(laf));
	...
	public JMenuItem createMultiscreenMenuItem(JMenu menu, int screen) {
		...
		if (screen == MultiScreenAction.ALL_SCREENS) {
			...
			mi.getAccessibleContext()
					.setAccessibleDescription(getString("MultiMenu.single_accessible_description") + " " + screen);
	 ...
	 * @return the j menu item
	 */
	public JMenuItem createPopupMenuItem(JPopupMenu menu, String label, String mnemonic, String accessibleDescription,
			String laf) {
...
//		mi.setEnabled(isAvailableLookAndFeel(laf));
	...
	/**
	 * Load the first demo. This is done separately from the remaining demos so that
	 * we can get MySwingSet up and available to the user quickly.
	 */
	public void preloadFirstDemo() {
		MyDemoModule demo = addDemo(new MyInternalFrameDemo(this));
		setDemo(demo);
	}
	 ...
	 */
	public MyDemoModule addDemo(MyDemoModule demo) {
		demosList.add(demo);
		if (dragEnabled) {
		...
		SwingUtilities.invokeLater(new SwingSetRunnable(this, demo) {
			public void run() {
				SwitchToDemoAction action = new SwitchToDemoAction(swingset, (MyDemoModule) obj);
...
//				tb.setText(null);
				tb.setIcon(null);
				tb.setToolTipText(((MyDemoModule) obj).getToolTip());

				if (demos.length>0&&demos[demos.length - 1].equals(obj.getClass().getName())) {
	 ...
	 */
	public void setDemo(MyDemoModule demo) {
		currentDemo = demo;
		...
		// ** ，当然该段代码不能算作bug，只是MySwingSet为了确保演示L&F时能即时刷新UI，但恰好跟jb2011需要的目标不同而已）！
//		SwingUtilities.updateComponentTreeUI(currentDemoPanel);
	...
	/**
	 * Bring up the MySwingSet demo by showing the frame (only applicable if coming
	 * up as an application, not an applet);.
	 */
	public void showMySwingSet() {
...
//			f.pack();
	...
	void loadDemo(String classname) {
		...
		MyDemoModule demo = null;
		try {
			Class demoClass = Class.forName(classname);
			Constructor demoConstructor = demoClass.getConstructor(new Class[] { MySwingEntry.class });
			demo = (MyDemoModule) demoConstructor.newInstance(new Object[] { this });
			addDemo(demo);
		} catch (Exception e) {
	 ...
	 * is any kind of error checking if the LookAndFeel is supported.
	 * 
	 * The L&F menu will use this method to detemine whether the various L&F options
	 ...
	 * @return the applet
	 */
	public SwingSetBaseApplet getApplet() {
		return applet;
	}
	...
	/**
	 * Create a frame for MySwingSet to reside in if brought up as an application.
	 *
	 * @param gc the gc
	 * @return the j frame
	 */
	 ...
	 */
	public void setSourceCode(MyDemoModule demo) {
		// do the following on the gui thread
		SwingUtilities.invokeLater(new SwingSetRunnable(this, demo) {
			public void run() {
				swingset.demoSrcPane.setText(((MyDemoModule) obj).getSourceCode());
...
//			tb.setText(null);
	...
	/**
	 * Generic MySwingSet runnable. This is intended to run on the AWT gui event
	 * thread so as not to muck things up by doing gui work off the gui thread.
	 * Accepts a MySwingSet and an Object as arguments, which gives subtypes of this
	 * class the two "must haves" needed in most runnables for this demo.
	 */
	class SwingSetRunnable implements Runnable {

		/** The swingset. */
		protected MySwingEntry swingset;
		 ...
		 * @param obj      the obj
		 */
		public SwingSetRunnable(MySwingEntry swingset, Object obj) {
	...
	public class SwitchToDemoAction extends AbstractAction {
		 ...
		 * @param demo     the demo
		 */
		public SwitchToDemoAction(MySwingEntry swingset, MyDemoModule demo) {
	...
	class ChangeLookAndFeelAction extends AbstractAction {
		 ...
		 * @param laf      the laf
		 */
		protected ChangeLookAndFeelAction(MySwingEntry swingset, String laf) {
			super("ChangeTheme");
			this.swingset = swingset;
			this.laf = laf;
		}
	...
	class ActivatePopupMenuAction extends AbstractAction {
		 ...
		 */
		protected ActivatePopupMenuAction(MySwingEntry swingset, JPopupMenu popup) {
	...
	class OnAudioAction extends AbstractAction {
		 ...
		 * @param swingset the swingset
		 */
		protected OnAudioAction(MySwingEntry swingset) {
			super("Audio On");
			this.swingset = swingset;
		}
...
//			swingset.updateLookAndFeel();
	...
	class DefaultAudioAction extends AbstractAction {
		 ...
		 * @param swingset the swingset
		 */
		protected DefaultAudioAction(MySwingEntry swingset) {
			super("Audio Default");
			this.swingset = swingset;
		}
...
//			swingset.updateLookAndFeel();
	...
	class OffAudioAction extends AbstractAction {
		 ...
		 * @param swingset the swingset
		 */
		protected OffAudioAction(MySwingEntry swingset) {
			super("Audio Off");
			this.swingset = swingset;
		}
...
//			swingset.updateLookAndFeel();
	...
	class ChangeThemeAction extends AbstractAction {
		 ...
		 * @param theme    the theme
		 */
		protected ChangeThemeAction(MySwingEntry swingset, MetalTheme theme) {
			super("ChangeTheme");
			this.swingset = swingset;
			this.theme = theme;
		}
	...
	class ExitAction extends AbstractAction {
		 ...
		 * @param swingset the swingset
		 */
		protected ExitAction(MySwingEntry swingset) {
			super("ExitAction");
			this.swingset = swingset;
		}
	...
	class AboutAction extends AbstractAction {
		 ...
		 * @param swingset the swingset
		 */
		protected AboutAction(MySwingEntry swingset) {
			super("AboutAction");
			this.swingset = swingset;
		}
	...
	class MultiScreenAction extends AbstractAction {
		 ...
		 * @param screen   the screen
		 */
		protected MultiScreenAction(MySwingEntry swingset, int screen) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (screen == ALL_SCREENS) {
				for (int i = 0; i < gds.length; i++) {
					MySwingEntry swingset = new MySwingEntry(null, gds[i].getDefaultConfiguration());
					swingset.setDragEnabled(dragEnabled);
				}
			} else {
				MySwingEntry swingset = new MySwingEntry(null, gds[screen].getDefaultConfiguration());
				swingset.setDragEnabled(dragEnabled);
			}
		}
	}
	...
	class DemoLoadThread extends Thread {
		 ...
		 * @param swingset the swingset
		 */
		public DemoLoadThread(MySwingEntry swingset) {
			this.swingset = swingset;
		}
	...
	class AboutPanel extends JPanel {
		 ...
		 * @param swingset the swingset
		 */
		public AboutPanel(MySwingEntry swingset) {
			this.swingset = swingset;
	...
	private class ChangeFontAction extends AbstractAction {
		 ...
		 */
		ChangeFontAction(MySwingEntry swingset, boolean plain) {
			super("FontMenu");
			this.swingset = swingset;
			this.plain = plain;
		}
